...
import os
from helpers import parse_propfind
...
def modify_request():
    ...
    if request.method == 'PROPFIND':
        ...
        # print("Modified RURI: " + str(request.url))
            return redirect(url_for('methods_handler'))

    if debug:
        with open(os.path.join(os.path.dirname(os.path.abspath(__file__)), request_log),mode='w') as file:
            file.write("-----\n")
...
def logging(response):
    if debug:
        with open(os.path.join(os.path.dirname(os.path.abspath(__file__)), response_log),mode='w') as file:
            file.write("-----\n")
